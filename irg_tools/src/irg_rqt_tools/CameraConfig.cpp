...
namespace irg_rqt_tools {
  ...
  void CameraConfig::initPlugin(qt_gui_cpp::PluginContext& context)
  {
    ...
    m_ui.set_adc_bits_button->setIcon(QIcon::fromTheme("view-refresh"));
    m_ui.set_adc_bits_button->setText("");
    m_ui.set_energy_conversion_button->setIcon(QIcon::fromTheme("view-refresh"));
    m_ui.set_energy_conversion_button->setText("");
    m_ui.set_exposure_button->setIcon(QIcon::fromTheme("view-refresh"));
    m_ui.set_exposure_button->setText("");
    m_ui.set_gain_button->setIcon(QIcon::fromTheme("view-refresh"));
    m_ui.set_gain_button->setText("");
    m_ui.set_gamma_button->setIcon(QIcon::fromTheme("view-refresh"));
    m_ui.set_gamma_button->setText("");
    m_ui.set_read_noise_button->setIcon(QIcon::fromTheme("view-refresh"));
    m_ui.set_read_noise_button->setText("");
    m_ui.set_shot_noise_button->setIcon(QIcon::fromTheme("view-refresh"));
  ...
  {
    auto current_camera = instanceSettings.value("current_camera", "").toString();
    selectCamera(current_camera);
  }
  ...
  void CameraConfig::onCameraChanged(int index)
  {
    ...
    for (auto & parameter : parameters) {
      ...
      } else if (parameter.get_name() == "adc_bits") {
  ...
  void CameraConfig::onRestoreDefaultsClicked()
  {
    ...
    auto result = rclcpp::executors::spin_node_until_future_complete(m_executor, m_paramClientNode, future, 1s);
  ...
  {
    setDoubleParameter("adc_bits", m_ui.adc_bits_edit->text().toDouble());
  }

  void CameraConfig::onSetEnergyConversionClicked()
  {
    setDoubleParameter("energy_conversion", m_ui.energy_conversion_edit->text().toDouble());
  }

  void CameraConfig::onSetExposureClicked()
  {
    setDoubleParameter("exposure", m_ui.exposure_edit->text().toDouble());
  }

  void CameraConfig::onSetGainClicked()
  {
    setDoubleParameter("gain", m_ui.gain_edit->text().toDouble());
  }

  void CameraConfig::onSetGammaClicked()
  {
    setDoubleParameter("gamma", m_ui.gamma_edit->text().toDouble());
  }

  void CameraConfig::onSetReadNoiseClicked()
  {
    setDoubleParameter("read_noise", m_ui.read_noise_edit->text().toDouble());
  }

  void CameraConfig::onSetShotNoiseClicked()
  {
    setDoubleParameter("shot_noise", m_ui.shot_noise_edit->text().toDouble());
  }
